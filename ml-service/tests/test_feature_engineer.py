...
class TestFeatureEngineer:
    ...
    @pytest.fixture
    def feature_engineer(self, mock_scaler):
        with patch("app.services.feature_engineer.open", mock_open()):
    ...
    def test_output_shape(self, feature_engineer):
        np.random.seed(42)
        gaze_points = create_gaze_points(100, reading_pattern=False)

        result = feature_engineer.process_gaze_points(gaze_points, 1920, 1080)

        assert result.shape == (100, 20, 5)
        assert result.dtype == np.float32

    def test_output_padded_when_few_sequences(self, feature_engineer):
        np.random.seed(42)
    ...
    def test_raises_on_insufficient_points(self, feature_engineer):
        gaze_points = create_gaze_points(15, reading_pattern=False)

        with pytest.raises(ValueError, match="Insufficient gaze points"):
            feature_engineer.process_gaze_points(gaze_points, 1920, 1080)

    def test_raises_when_all_fixations_filtered(self, feature_engineer):
        ...
        with pytest.raises(ValueError, match="valid fixations remain"):
            feature_engineer.process_gaze_points(points, 1920, 1080)

    # --- Saccade Amplitude Calculation ---

    def test_saccade_amplitudes_horizontal(self, feature_engineer):
        x = np.array([0.0, 0.1, 0.2])
        y = np.array([0.0, 0.0, 0.0])

        amplitudes = feature_engineer._calculate_saccade_amplitudes(x, y, 1000, 1000)
    ...
    def test_saccade_amplitudes_diagonal(self, feature_engineer):
        x = np.array([0.0, 0.1])
        y = np.array([0.0, 0.1])
    ...
    def test_saccade_velocities(self, feature_engineer):
        amplitudes = np.array([0.0, 100.0, 200.0])
        durations = np.array([150.0, 150.0, 100.0])

        velocities = feature_engineer._calculate_saccade_velocities(
    ...
    def test_saccade_velocities_zero_duration(self, feature_engineer):
    ...
    def test_create_sequences_sliding_window(self, feature_engineer):
        features = np.random.randn(30, 5)

        sequences = feature_engineer._create_sequences(features)

        # (30-20)/5 + 1 = 3 sequences
        assert sequences.shape == (3, 20, 5)

    def test_create_sequences_preserves_data(self, feature_engineer):
    ...
    def test_pad_sequences_pads_short_input(self, feature_engineer):
        sequences = np.ones((10, 20, 5), dtype=np.float32)

        padded = feature_engineer._pad_sequences(sequences)

        assert padded.shape == (100, 20, 5)
        np.testing.assert_array_equal(padded[:10], sequences)
        np.testing.assert_array_equal(padded[10:], 0)

    def test_pad_sequences_truncates_long_input(self, feature_engineer):
    ...
    def test_screen_size_affects_amplitudes(self, feature_engineer):
        np.random.seed(42)
        gaze_points = create_gaze_points(100, reading_pattern=False)

        result_small = feature_engineer.process_gaze_points(gaze_points, 800, 600)
        result_large = feature_engineer.process_gaze_points(gaze_points, 2560, 1440)
