...
class TaskProcessingResult:
    """Processed task data returned by the eye-tracker feature pipeline."""
...
class EyeTrackerFeatureProcessor:
    """Feature engineering for Tobii eye-tracker task streams."""
    ...
    def _calculate_saccade_amplitudes(
        ...
        """Compute saccade amplitudes in pixels between consecutive fixations."""
        x_px = x_coords * screen_width
        y_px = y_coords * screen_height
    ...
    def _calculate_saccade_velocities(
        ...
        """Compute per-fixation velocity from amplitude and duration."""
    ...
    def _create_sequences(self, features: np.ndarray) -> np.ndarray:
        """Create overlapping fixed-length sequence windows for the model."""
        sequences = []
    ...
    def _pad_sequences(self, sequences: np.ndarray) -> np.ndarray:
        """Pad or truncate task sequences to the configured model count."""
