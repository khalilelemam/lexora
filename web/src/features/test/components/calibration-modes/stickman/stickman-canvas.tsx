...
export function StickmanCanvas({
  ...
  const generatePath = useCallback((
    ...
    for (let i = 1; i < pts.length; i++) {
      if (i === pts.length - 1) {
        // Final: slow walk in
        waypoints.push({ x: pts[i].x, y: pts[i].y, duration: pace(700), type: 'walk', pose: 'idle' });
      } else if (i === pts.length - 2) {
        // Near approach: walk
        waypoints.push({ x: pts[i].x, y: pts[i].y, duration: pace(500), pause: pace(100), type: 'walk' });
      } else {
        // Exploration: sprint → skid on arrival
        waypoints.push({ x: pts[i].x, y: pts[i].y, duration: pace(300), pause: pace(150), type: 'dash', pose: 'skid' });
      }
    }
  ...
  const initGame = useCallback(() => {
    ...
    gameRef.current = {
      phase: 'sequence', waypoints, currentWpIndex: 0,
      wpStartTime: performance.now(),
      startX, startY, stickX: startX, stickY: startY,
      stickAngle: 0, pose: 'idle', facingRight: true,
      scanProgress: 0, screenShake: 0,
      lastHitTime: 0, phaseStartTime: performance.now(),
      particles: [], speedLines: [], lastStepTime: 0,
      sampleCollectedForThisPoint: false, width: w, height: h, dpr,
    };
      ...
      if (!wp) {
        ...
        if (elapsed < wp.duration) {
          ...
          if (wp.type === 'walk') {
            ...
            if (time - game.lastStepTime > 320) { audio.play('step'); game.lastStepTime = time; }
          } else {
            game.pose = 'sprint';
          ...
          {Array.from({ length: collectionTotal }).map((_, idx) => (
            <div
              ...
              className={cn(
                ...
                idx < activeIdx
                  ? 'w-2 h-2 bg-red-500/70'
                  : idx === activeIdx
                    ? 'w-2.5 h-2.5 bg-red-500 shadow-[0_0_5px_rgba(220,38,38,0.5)]'
